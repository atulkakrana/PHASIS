...
# this verusion have't fully tested for the est and cDNA subjects.
...
# 2013-06-21 v.30 change the data struture to speed up the calc. use the Array to replace the hash for p-value calc in a sliding windows in terms of a certain sRNA
# 2013-06-23 v.31 finish the single thread support for the scoring
# 2013-06-24 v.32 add the boundary informaiton to a hash  for the PARE useness
# 2013-07-11 v.341 bug fix,  the default of value for n and t should be added the 'defined' 
# 2013-07-12 v.342 feature added, add bowtie alignment reports limitation by -k options
...
if ($pare_data) {# the pare format is tag only
	...
	while (<PAREOUT>) {
		...
		$PARE_alignment{$target}->{$pos_new}->{$strand}->{'abun'} += $abun;
		$PARE_alignment{$target}->{$pos_new}->{$strand}->{'seq'}   = pare_seq_combine($PARE_alignment{$target}->{$pos_new}->{$strand}->{'seq'}, $seq_new);
	}
	close PAREOUT;
	...
	foreach my $target (sort {$a cmp $b} keys %PARE_alignment) {
		foreach my $pos (sort {$a <=> $b} keys %{$PARE_alignment{$target}}) {
			foreach my $strand (keys %{$PARE_alignment{$target}->{$pos}}) {
			
				my $abun = ($PARE_alignment{$target}->{$pos}->{$strand}->{'abun'})?($PARE_alignment{$target}->{$pos}->{$strand}->{'abun'}):"NA";
				my $seq  = ($PARE_alignment{$target}->{$pos}->{$strand}->{'seq'})?$PARE_alignment{$target}->{$pos}->{$strand}->{'seq'}:"NA";
				print PARE_PARSE "$target\t$pos\t$strand\t$abun\t$seq\n";
			}
		}
	}
	close PARE_PARSE;
	...
	foreach my $rl (sort {$a <=> $b} keys %boundary) {
			foreach my $cut (sort {$a <=> $b} keys %{$boundary{$rl}}) {
				foreach my $target (sort {$a cmp $b} keys %{$boundary{$rl}->{$cut}}) {
					my @cluster_ref =@{$boundary{$rl}->{$cut}->{$target}};
					while (my ($idx,$region_ref) = each @cluster_ref) {
						...
						my @top_three_pare_ref = &top_pare($target,$cluster_start - $rl*4 - 2, $cluster_end + $rl*4 + 2, 3);
							
						# the stastics the 96.9% pare located within 3 ~ 4 sRNA size
						my $total_pare_abun_in_an_wider_range = pare_abun_total($target, $cluster_start - $rl*4 - 2, $cluster_end + $rl*4 + 2); # incase of 2 nt overhang, 4 register length plus the 2nt overhang will be the maximal range to detect the total amount of the PARE signatures
						my $prime_5_outer_abun                = pare_abun_total($target, $cluster_start - $rl*4 - 2, $cluster_start + $rl + 2);
						my $prime_3_outer_abun                = pare_abun_total($target, $cluster_end - $rl -2, $cluster_end + $rl*4 + 2);
						...
						if ($total_pare_abun_in_an_wider_range > 0) {
							my $motif_prime;
							if ($prime_5_outer_abun >= 3 or $prime_3_outer_abun>= 3) {
									...
									if ($prime_5_outer_abun == $prime_3_outer_abun) {
										...
										push(@{$miscandidate{$rl}->{$cut}->{$target}},[$cluster_start, $cluster_end,'UC','UC']);
									}
									else {
										
										my @a1 = top_pare($target,  $cluster_start - $rl*4 - 2, $cluster_start + $rl   + 2, 1);
										my @a2 = top_pare($target,  $cluster_end   - $rl   - 2, $cluster_end   + $rl*4 + 2, 1);
										...
										#print "$j5,$j3,$flag1,$flag2\n";
										
										if ($j5 + $j3 + $flag1 + $flag2 == 4) {
											#two?
											push(@{$miscandidate{$rl}->{$cut}->{$target}},[$cluster_start, $cluster_end,'h2','h2']); # either top3 or phased or not
										}
											...
											push(@{$miscandidate{$rl}->{$cut}->{$target}},[$cluster_start, $cluster_end,'tpon','tpon']); # either top3 or phased or not
										}
									}
							}
							else{
								#seems that there is no pare located in the two regions, perhaps this is not a PHAS locus
								push(@{$miscandidate{$rl}->{$cut}->{$target}},[$cluster_start, $cluster_end,'lt3','lt3']);
							}						
						}
						else {
							push(@{$miscandidate{$rl}->{$cut}->{$target}},[$cluster_start, $cluster_end,'tlt0','tlt0']); #total less than 10
						}
					}
				}
			}
	...
	foreach my $rl (sort {$a<=>$b} keys %candidate) { 
		foreach my $cut (sort {$a <=> $b} keys %{$candidate{$rl}}) {
			foreach my $chr (sort {$a cmp $b} keys %{$candidate{$rl}->{$cut}}) {
	...
	foreach my $rl (sort {$a<=>$b} keys %miscandidate) { #push(@{$miscandidate{$rl}->{$cut}->{$chr}},[$cluster_start, $cluster_end]);
		foreach my $cut (sort {$a <=> $b} keys %{$miscandidate{$rl}}) {
			foreach my $chr (sort {$a cmp $b} keys %{$miscandidate{$rl}->{$cut}}) {
